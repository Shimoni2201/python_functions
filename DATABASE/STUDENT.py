...
while op1 !=6 :
    ...
    if op1 == 1:
        ...
        for row in cur:
            ...
            rid=int(input("enter the roll no. you want to update"))
            rno=int(input("enter roll no:"))
            ...
            cur.execute("update student set rollno=%s, sname=%s, maths=%s, sci=%s, eng=%s where rollno=%s",(rno,sname,math,sci,eng,rid))
            cur.execute(
                "select rollno,sname,maths,sci,eng,maths+sci+eng, case(maths>35 and sci>35 and eng>35) when true then 'pass' else 'fail' end from student ")
            for row in cur:
                for value in row:
                    print(value, end="\t")
                print("")
        ...
        print("\t delete")
        cur.execute("delete from student where rollno=%s",(rid,))
        cur.execute(
            "select rollno,sname,maths,sci,eng,maths+sci+eng, case(maths>35 and sci>35 and eng>35) when true then 'pass' else 'fail' end from student ")
        for row in cur:
            for value in row:
